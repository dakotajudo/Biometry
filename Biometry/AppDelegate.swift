...
class AppDelegate: NSObject, NSApplicationDelegate {
   ...
   lazy var applicationDocumentsDirectory: NSURL = {
       // The directory the application uses to store the Core Data store file. This code uses a directory named "Dakota-Judo.Biometry" in the user's Application Support directory.
       let urls = NSFileManager.defaultManager().URLsForDirectory(.ApplicationSupportDirectory, inDomains: .UserDomainMask)
       let appSupportURL = urls[urls.count - 1] as NSURL
       return appSupportURL.URLByAppendingPathComponent("Dakota-Judo.Biometry")
   }()

   lazy var managedObjectModel: NSManagedObjectModel = {
       // The managed object model for the application. This property is not optional. It is a fatal error for the application not to be able to find and load its model.
       let modelURL = NSBundle.mainBundle().URLForResource("Biometry", withExtension: "momd")!
   ...
   lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
       ...
       if !shouldFail && (error == nil) {
           ...
           let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("Biometry.storedata")
